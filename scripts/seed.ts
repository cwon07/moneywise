...
const SEED_USER_ID = "user_2jvkD1kudPS6FHkR42aqVh1dPAe"
...
const generateRandomAmount = (category: typeof categories.$inferInsert) => {
    switch (category.name) {
        case "Rent":
            return Math.random() * 400 + 90; // as rent will likely be a larger amount
        case "Utilities":
...
const generateTransactionsForDay = (day: Date) => {
    ...
    for (let i = 0; i < numTransactions; i++) {
        ...
        SEED_TRANSACTIONS.push({
            ...
            accountId: SEED_ACCOUNTS[0].id, // assumes always using the first account for simplicity's sake
